...
export class Player {
    ...
    horizontalMove(movementValue) {
        ...
        if (next + PLAYER_WIDTH > this.width) return this.x = this.width - PLAYER_WIDTH;
        this.x = next;
    }
    verticalMove(movementValue) {
        ...
        if (next + PLAYER_HEIGHT > this.height) return this.y = this.height - PLAYER_HEIGHT;
        this.y = next;
    }
    // Actor collision with Mob needs to be done, so some extra interface will be here
}
